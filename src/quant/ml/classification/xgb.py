...
def main(interval: str):
    ...
    spec = TripleBarrierSpec(H=5, pt_k=1.0, sl_k=1.0, vol_method=VolMethod.ATR, entry=EntryPriceMode.NEXT_OPEN)
    # 1. Load Data
    ...
    assert not np.isinf(df.select_dtypes(include=np.number)).values.any(), "Data still contains INF!"
    ...
    test_mask = (df[BronzeColumns.DATE.value] > cutoff_date) & (df[BronzeColumns.DATE.value] <= '2025-12-31')
...
if __name__ == "__main__":
    import argparse 
    parser = argparse.ArgumentParser(description="Train a global XGBoost model on the silver dataset.")
    # load interval 
    parser.add_argument('--interval', type=str, default='day', help='Interval of the data to use (e.g., day, 60minute)')
