...
def plot_predictions_vs_actual(
    results_df, model_name="NBEATSx", hyperparams=None, title_suffix="", save_path=""
):
    """
    ...
    if not save_path:
        ...
        if hyperparams:
            hyper_str = "_" + "_".join(
                [f"{k}-{v}" for k, v in hyperparams.items() if k in ["input_size", "window_size"]]
            )
    ...
    for stock in unique_stocks:
        ...
        plt.title(
            f"Actual vs predictions for {stock} {title_suffix}\nMAE: {mae:.4f}, RMSE: {rmse:.4f}"
        )
        plt.xlabel("Date")
...
def rolling_forecast_evaluation(
    ...
    while current_end_training_window_idx + window_size - 1 < len(unique_dates):
        ...
        logger.info(
            f"Processing training window: {train_window_start} to {train_window_end}, test window end: {test_window_end}"
        )

        train_df = df[(df["ds"] >= train_window_start) & (df["ds"] < train_window_end)].copy()

        assert (
            train_df["ds"].nunique() == input_size
        ), f"Expected {input_size} unique dates in training data, got {train_df['ds'].nunique()}"

        test_df = df[(df["ds"] >= train_window_end) & (df["ds"] <= test_window_end)].copy()
        assert (
            test_df["ds"].nunique() == window_size
        ), f"Expected {window_size} unique dates in test data, got {test_df['ds'].nunique()}"
        ...
        try:
            logger.info(
                f"Training window starting {train_window_start} to {train_window_end} with {len(train_df)} training samples"
            )
        ...
        if not merged.empty:
            ...
            metrics.append(
                {
                    "window_start": train_window_start,
...
def train(
    ...
    df = df.rename(columns={"date": "ds", "symbol": "unique_id", target: "y"})
    # Ensure ds tz-naive
    df["ds"] = pd.to_datetime(df["ds"]).dt.tz_localize(None)
    ...
    for stock in df.unique_id.unique():
        ...
        if key in scalers and scalers[key] is not None:
            results.loc[mask, "y"] = (
                scalers[key]
                .inverse_transform(results.loc[mask, "y"].values.reshape(-1, 1))
                .flatten()
            )
            if "NBEATSx" in results.columns:
                results.loc[mask, "predictions"] = (
                    scalers[key]
                    .inverse_transform(results.loc[mask, "NBEATSx"].values.reshape(-1, 1))
                    .flatten()
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Index to train on (e.g., 'nifty_50', 'banknifty', 'midcap_150')",
    )
    parser.add_argument(
        "--interval", type=str, required=True, help="Data interval (e.g., 'day', 'hour', etc.)"
    )
    parser.add_argument(
        "--cutoff_date",
        type=str,
    ...
    train(index=args.index, interval=args.interval, target=args.target)
