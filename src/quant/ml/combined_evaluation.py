...
for model_name, func in methods.items():
    for uid, train in trains.items():
        ...
        test_raw = (
            pd.Series(
                scaler.inverse_transform(test.values.reshape(-1, 1)).flatten(), index=test.index
            )
            if scaler
            else test
        )
        ...
        for ds, y_pred in preds.items():
            records.append({"unique_id": uid, "ds": ds, "model": model_name, "prediction": y_pred})

base_preds = pd.DataFrame(records)

# 6. Extract actuals for test period
actuals = raw_df[raw_df["ds"] >= SPLIT_DATE][["unique_id", "ds", "y"]].rename(
    columns={"y": "actual"}
)
