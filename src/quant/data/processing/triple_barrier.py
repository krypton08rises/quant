...
from quant.data.kite_handler import KiteDataHandler
from quant.data._common import *  
from quant.data.indicators import generate_indicators_from_df

def compute_entry_price(
    df: pd.DataFrame, t: int, spec: TripleBarrierSpec) -> float:
    """
    ...
    """
    assert "open" in df.columns and "close" in df.columns, f"DataFrame must contain 'open' and 'close' columns. Only have: {df.columns.tolist()}"
...
def label_at_t(
    ...
    if t + spec.H >= len(df):
        logger.warning(f"Not enough future data to apply vertical barrier of H={spec.H} days at index {t}. Only have {len(df)-t-1} days ahead. Date is {df[BronzeColumns.DATE].iloc[t]}.")
    ...
    for i in range(start_idx, end_idx):
        ...
        if spec.use_high_low:
            if high >= pt_barrier and low <= sl_barrier:
                ...
                if spec.tie_breaking == TieBreaking.CONSERVATIVE.value:
                    return -1  # treat as stop loss hit
                elif spec.tie_breaking == TieBreaking.ZERO.value:
...
def generate_static_dataset(
    ...
    logger.info(f"Loaded bronze data for {number_of_symbols} unique symbols across indices at interval {interval.value}.")
    ...
    def process_symbol(sym_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for t in tqdm(range(len(sym_df) - spec.H - 1), desc=f"Labeling Symbol {sym_df[BronzeColumns.SYMBOL.value].iloc[0]} for H={spec.H}"):
            label = label_at_t(sym_df, t, spec)
            sym_df.loc[t, f"label_{spec.H}"] = label
        return sym_df     

    try:
        specs = [TripleBarrierSpec(H=5), TripleBarrierSpec(H=7)]
        df_labelled = df.groupby(BronzeColumns.SYMBOL, as_index=False).apply(process_symbol).reset_index(drop=True)
        assert BronzeColumns.SYMBOL.value in df_labelled.columns, "After labelling, SYMBOL column must exist."
...
def main():
    ...
    test_path = Path(f"{SILVER_DIR}/test_{Interval.DAY.value}_{spec.H}_{spec.pt_k}_{spec.sl_k}.parquet")
    embargoed_path = Path(f"{SILVER_DIR}/embargoed_{Interval.DAY.value}_{spec.H}_{spec.pt_k}_{spec.sl_k}.parquet")

    
    train_df = df_static[df_static[BronzeColumns.DATE.value] <= CUTOFF_DATE]
    test_df = df_static[(df_static[BronzeColumns.DATE.value] > CUTOFF_DATE) & (df_static[BronzeColumns.DATE.value] <= EMBARGOED_DATE_START) ]
