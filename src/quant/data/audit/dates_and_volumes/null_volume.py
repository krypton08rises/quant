...
def find_null_volume_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    null_volume_df = df[df[RawColumns.VOLUME.value] == 0]
    return null_volume_df


def main(interval: Interval = Interval.DAY) -> None:
    """
    Parse through all raw data files for the given interval and report zero-volume bars.
    """
    for pth in Path(RAW_DIR).glob(f"*_{interval.value}.pkl"):
        ...
        volume_min = df.groupby(RawColumns.SYMBOL.value)[RawColumns.VOLUME.value].min()
        ...
        # zero_volume_dates = df[df[RawColumns.VOLUME.value] == 0][RawColumns.DATE.value]
