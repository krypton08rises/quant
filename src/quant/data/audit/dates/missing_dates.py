...
def find_missing_dates(df: pd.Series, date_col: BronzeColumns) -> pd.DatetimeIndex:
    ...
    all_dates = pd.date_range(
        start=df[date_col.value].min(), end=df[date_col.value].max(), freq="B"
    )  # Business days only
...
def remove_phantom_ticks(
    ...
    if big_gaps.empty:
        logger.info(
            "No phantom ticks detected based on the current threshold. Returning original DataFrame."
        )
...
def filter_active_universe(
    ...
    logger.info(
        f"Filtering universe to {len(active_symbols)} active stocks with trading data in 2026."
    )
    ...
    for date, count in dates_vc.items():
        ...
        if active_symbols and count / active_symbols >= 0.9:
            ...
            if not active_symbols:
                logger.warning(
                    f"Date {date.date()} is missing for {count} symbols but only {active_symbols} were active. This may indicate a data issue rather than a holiday."
                )
    logger.info(
        f"Total unresolved missing dates (potential data issues): {unresolved} out of {len(dates_vc)} total missing dates. | Identified holidays: {identified}"
    )
...
def main():
    for pth in Path(BRONZE_DIR).glob("*day.pkl"):
        ...
        missing_dates = df.groupby(BronzeColumns.SYMBOL.value).apply(
            lambda x: find_missing_dates(x, date_col=BronzeColumns.DATE)
        )
        holidays, unresolved_dates = get_market_holidays(df, missing_dates)

        # 4. Audit what's left
        audit_data_quality(missing_dates, holidays)
